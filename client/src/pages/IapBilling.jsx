...
export default function IapBilling() {
  ...
  if (loading) {
    return <div className="billing-page"><div className="loading">Načítavam…</div></div>;
  }

  // Iba team + pro sú kúpiteľné (free je default). Zoradíme team, pro.
  const buyablePlans = plans.filter((p) => p.id === 'team' || p.id === 'pro');

  return (
    <div className="billing-page">
      <div className="billing-header" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '16px' }}>
        <button className="btn btn-secondary" style={{ fontSize: 14 }} onClick={() => navigate('/app')}>← Späť</button>
        <h1 style={{ fontSize: 20, fontWeight: 700, margin: 0 }}>Predplatné</h1>
      </div>

      <div className="billing-content" style={{ padding: '0 16px 32px' }}>
        {/* Aktuálny plán */}
        ...
        {/* Apple-managed info — predplatné sa spravuje v iOS Settings */}
        {appleManaged && (
        ...
        <div style={{ textAlign: 'center', marginTop: 24 }}>
          ...
          <p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 16, lineHeight: 1.6, maxWidth: 480, margin: '16px auto 0' }}>
