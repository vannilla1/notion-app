...
- `models/Notification.test.js` — Notification model (38 testov: 20 typov enum validácia,
  relatedType enum, TTL expiresAt +30 dní, P2 Workspace Isolation, legacy workspaceId
  optional podpora, data mixed payload)
- `models/APNsDevice.test.js` — APNs push device (11 testov: unique deviceToken, viacero
  zariadení per user iPhone+iPad, environment enum production|sandbox|null, cleanup
  scenario)
- `models/PromoCode.test.js` — Billing promo kódy (26 testov: auto-uppercase + trim code,
  type enum percentage|fixed|freeMonths, instance methods isValid/canBeUsedBy/
  isValidForPlan/calculateDiscount vrátane rounding a capping edge cases)
- `models/AuditLog.test.js` — Admin audit trail (17 testov: 8-hodnotový category enum,
  90-dňový TTL index, mixed details payload, workspace-less system actions)
...
- `models/AuditLog.js` — duplikovaný index na `createdAt` (`index: true` v schéme +
  `schema.index({ createdAt: 1 }, { expireAfterSeconds: 90d })` ako TTL). Mongoose
  skúšal vytvoriť 2 indexy s rovnakým názvom `createdAt_1` ale rôznymi opciami →
  MongoServerError. V produkčnom MongoDB to mohlo spôsobiť, že **TTL sa nevytvoril
  a audit logy sa nikdy samy nemazali** (risk pre GDPR compliance).
- `models/Invitation.js` — rovnaký vzor na `token`: `unique: true` v schéme vytvára
  `token_1` unique index + samostatný duplicitný `schema.index({ token: 1 })`.
  Fix: odstránený redundantný manuálny index.
